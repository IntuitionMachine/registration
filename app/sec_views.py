...
class MyUserDBModelView(UserDBModelView):
	show_fieldsets = [(lazy_gettext('User Profile Dawg'),{'fields': ['username', 'active', 'roles', 'login_count', 'birthday','streetAddress','country','city','state','zipCode','ethAddress','govID','geoIP','geoLoc']}),(lazy_gettext('Personal Info'),{'fields': ['first_name', 'last_name', 'email'], 'expanded': True}),(lazy_gettext('Audit Info'),{'fields': ['last_login', 'fail_login_count', 'created_on','created_by', 'changed_on', 'changed_by'], 'expanded': False})]
...
class MyRegisterUserDBView(BaseRegisterUser):
	...
	#redirect_url= '/'
	@staticmethod
	def chat_register(self,username,email,first_name,last_name,password):
		self.add_registration(username=username,
